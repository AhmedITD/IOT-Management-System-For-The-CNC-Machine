...
<div class="app">
    ...
    <form method="post" action="{{route('gcodes.update', ['gcode' => $gcode])}}" class="form" enctype="multipart/form-data">
    @csrf
    @method('put')
    <div class="p">
        <label for="input-username">Enter The username</label>
        <input type="text" name="name" class="username" id="input-username" value="{{$gcode->name}}">
    </div>
    <div class="p">   
        <label for="input-email">Enter The email</label>
        <input type="email" name="email" class="email" id="input-email" value="{{$gcode->email}}">
    </div>
    <div class="p">
